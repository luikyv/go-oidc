...
func TestNew(t *testing.T) {
	...
	if diff := cmp.Diff(
		...
		cmpopts.IgnoreFields(
			...
			"ClientManager",
			"AuthnSessionManager",
			"AuthnSessionByAuthCodeFunc",
			"AuthnSessionByPARIDFunc",
			"AuthnSessionByCIBAIDFunc",
			"GrantManager",
			"DeleteGrantByAuthCodeFunc",
			"TokenManager",
			"JWKSFunc",
			"TokenOptionsFunc",
...
func TestNew_WithOptions(t *testing.T) {
	...
	op, err := New(
		...
		issuer,
		jwksFunc,
		WithAuthorizationCodeGrant(nil, nil),
		WithImplicitGrant(),
		WithCIBAGrant(goidc.CIBAProfileOpenID, nil),
