...
func generateDeviceCodeGrant(ctx oidc.Context, req request) (response, error) {
	...
	grant, err := ctx.GrantByDeviceCode(req.code)
	if err != nil {
	...
	if c.ID != grant.ClientID {
		return response{}, goidc.NewError(goidc.ErrorCodeInvalidGrant, "the authorization code was not issued to the client")
	}
