...
func TestHandleGrantCreation_JWTBearerGrant(t *testing.T) {
	...
	tokenClaims, err := oidctest.SafeClaims(tokenResp.AccessToken, ctx.PrivateJWKS.Keys[0])
	if err != nil {
	...
	idTokenClaims, err := oidctest.SafeClaims(tokenResp.IDToken, ctx.UserSigKey())
	if err != nil {
		t.Fatalf("error parsing claims: %v", err)
	}
