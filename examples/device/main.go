// copied from oidc/main.go with the addition of the device authorization grant
package main
...
func main() {
	client, _ := authutil.ClientPrivateKeyJWT("client_one")
	client.TokenAuthnMethod = goidc.ClientAuthnNone
	client.GrantTypes = []goidc.GrantType{goidc.GrantDeviceCode}
	...
	op, err := provider.New(
		...
		provider.WithDeviceAuthorizationGrant(authutil.UserCodeHandler),
